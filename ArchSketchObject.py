...
class ArchSketch(ArchSketchObject):
    ...
    MasterSketchSubelementTags = [
        "MasterSketchSubelementTag",
    ...
    GeomSupported = (Part.LineSegment, Part.Circle, Part.ArcOfCircle, Part.Ellipse)
    ...
    def setProperties(self, fp):
        ...
        if not hasattr(fp, "Align"):
            fp.addProperty(
                ...
                "Align",
                ArchSkPropStr,
                "The default alignment for the wall with this ArchSketch as its base object",
            )
        ...
        if not hasattr(fp, "Offset"):
            fp.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "The offset between the wall (segment) and its baseline (only for left and right alignments)",
                ),
            )
        ...
        if not hasattr(fp, "DetectRoom"):
            fp.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "Enable to detect rooms enclosed by edges/walls - For CellComplex object to work, the generated shape is not shown in the ArchSketch object, but shown by a CellComplex Object.  This make recompute of this object longer !",
                ),
            )
        if not hasattr(fp, "CellComplexElements"):
            fp.addProperty(
                ...
                ArchSkPropStr,
                QT_TRANSLATE_NOOP(
                    "App::Property", "The Shape of built CellComplexElements"
                ),
                8,
            )
        if not hasattr(fp, "FloorHeight"):
            fp.addProperty(
                ...
                "FloorHeight",
                ArchSkPropStr,
                "Global ArchSketch Floor to Next Floor Height",
            )
        ...
        if not hasattr(fp, "PropertySet"):
            fp.addProperty(
                ...
                "PropertySet",
                ArchSkPropStr,
                QT_TRANSLATE_NOOP("App::Property", "List User Defined Property Sets"),
            )
        ...
        if not hasattr(fp, "ShapeList"):
            ...
            fp.addProperty(
                ...
                "ShapeList",
                ArchSkPropStr,
                QT_TRANSLATE_NOOP("App::Property", sstr),
                8,
            )
    ...
    def setPropertiesLinkCommon(self, orgFp, linkFp=None, mode=None):
        ...
        if "MasterSketchSubelementIndex" not in prop:
            fp.addProperty(
                "App::PropertyInteger",
                "MasterSketchSubelementIndex",
                "Referenced Object",
                "Index of MasterSketchSubelement to be synced on the fly.  For output only.",
                8,
            )
        ...
        if "MasterSketchIntersectingSubelementIndex" not in prop:
            fp.addProperty(
                "App::PropertyInteger",
                "MasterSketchIntersectingSubelementIndex",
                "Referenced Object",
                "Index of MasterSketchInteresctingSubelement to be synced on the fly.  For output only.",
                8,
            )
        ...
        # "Hosts" for Window
            ...
            if "Hosts" not in prop:
                fp.addProperty(
                    ...
                    "Window",
                    QT_TRANSLATE_NOOP(
                        "App::Property", "The objects that host this window"
                    ),
                )  # Arch Window's code
        ...
        if "MasterSketch" not in prop:
            fp.addProperty(
                ...
                "MasterSketch",
                "Referenced Object",
                "Master Sketch to Attach on",
            )
        if "MasterSketchSubelement" not in prop:
            fp.addProperty(
                "App::PropertyString",
                "MasterSketchSubelement",
                "Referenced Object",
                "Master Sketch Sub-Element to Attach on",
            )
        if "MasterSketchSubelementOffset" not in prop:
            fp.addProperty(
                "App::PropertyDistance",
                "MasterSketchSubelementOffset",
                "Referenced Object",
                "Master Sketch Sub-Element Attached Offset from Startpoint",
            )
        if "MasterSketchSubelementSnapPreset" not in prop:
            fp.addProperty(
                "App::PropertyEnumeration",
                "MasterSketchSubelementSnapPreset",
                "Referenced Object",
                "Preset Snap Offset from Axis Startpoint; will add ms-SubelementOffset Distance (mm)",
            )
            fp.MasterSketchSubelementSnapPreset = [
                "AxisStart",
                "1/4",
                "1/3",
                "MidPoint",
        ...
        if "MasterSketchSubelementSnapCustom" not in prop:
            fp.addProperty(
                ...
                "MasterSketchSubelementSnapCustom",
                "Referenced Object",
                "Custom Value: 0 to 1, Start/EndPoint of Axis (Use formula for fraction e.g 2/11",
            )
            fp.MasterSketchSubelementSnapCustom = (
                0.0,
                0.0,
                1.0,
                0.001,
            )  # (Default, Start, Finish, Step)
        if "MasterSketchIntersectingSubelement" not in prop:
            fp.addProperty(
                "App::PropertyString",
                "MasterSketchIntersectingSubelement",
                "Referenced Object",
                "Master Sketch Subelement Intersecting the Sub-Element Attached on",
            )
        if "AttachToSubelementOrOffset" not in prop:
            fp.addProperty(
                ...
                "Referenced Object",
                "Select MasterSketch Subelement or Specify Offset to Attach",
            )
            fp.AttachToSubelementOrOffset = [
                "Attach To Edge & Alignment",
                "Attach to Edge",
                "Follow Only Offset XYZ & Rotation",
            ]
        if "AttachmentOffsetXyzAndRotation" not in prop:
            fp.addProperty(
                ...
                "Referenced Object",
                "Specify XYZ and Rotation Offset",
            )
        if "AttachmentOffsetExtraRotation" not in prop:
            fp.addProperty(
                "App::PropertyEnumeration",
                "AttachmentOffsetExtraRotation",
                "Referenced Object",
                "Extra Rotation about X, Y or Z Axis",
            )
            fp.AttachmentOffsetExtraRotation = [
                "None",
                "X-Axis CW90",
                "X-Axis CCW90",
                "X-Axis CW180",
        ...
        if "OriginOffsetXyzAndRotation" not in prop:
            fp.addProperty(
                "App::PropertyPlacement",
                "OriginOffsetXyzAndRotation",
                "Referenced Object",
                "Specify Origin's XYZ and Rotation Offset",
            )
        if "FlipOffsetOriginToOtherEnd" not in prop:
            fp.addProperty(
                "App::PropertyBool",
                "FlipOffsetOriginToOtherEnd",
                "Referenced Object",
                "Flip Offset Origin to Other End of Edge / Wall ",
            )
        if "Flip180Degree" not in prop:
            fp.addProperty(
                ...
                "Flip180Degree",
                "Referenced Object",
                "Flip Orientation 180 Degree / Inside-Outside / Front-Back",
            )
        if "OffsetFromIntersectingSubelement" not in prop:
            fp.addProperty(
                ...
                "Referenced Object",
                "Offset from the Master Sketch Subelement Intersecting the Sub-Element to Attached on",
            )
        if "AttachmentAlignment" not in prop:
            fp.addProperty(
                ...
                "AttachmentAlignment",
                "Referenced Object",
                "If AttachToEdge&Alignment, Set (Wall)Left/Right to align to Edge of Wall",
            )
            ...
            if isinstance(fp.getLinkedObject().Proxy, ArchWindow._Window):
                ...
                fp.AttachmentAlignment = (
                    "WallLeft"  # default for cases other than Windows
                )
        if fp.AttachmentAlignment in [
            "Edge",
            "EdgeGroupWidthLeft",
            "EdgeGroupWidthRight",
        ]:
        ...
        if "AttachmentAlignmentOffset" not in prop:
            fp.addProperty(
                "App::PropertyDistance",
                "AttachmentAlignmentOffset",
                "Referenced Object",
                "Set Offset from Edge / EdgeGroupWidth +ve Right / -ve Left",
            )
        ...
        if "AttachToAxisOrSketch" in prop:
            ...
            fp.addProperty(
                "App::PropertyEnumeration",
                "AttachToAxisOrSketch",
                "Referenced Object",
                "Select Object Type to Attach on ",
            )
        ...
        # No existing selection, ie. newly added "AttachToAxisOrSketch" attribute
            ...
            fp.AttachToAxisOrSketch = (
                "Master Sketch"  # default option for ArchSketch + Link to ArchSketch
            )
            ...
            if mode == "ODR":
                ...
                fp.AttachToAxisOrSketch = (
                    "Host"  # default option for Arch Objects in general
                )
    ...
    def execute(self, fp):
        ...
        if True:
            ...
            propSetSelectedNameCur = self.getPropertySet(
                fp, propSetUuid=propSetPickedUuidPrev
            )
        ...
        if fp.DetectRoom:
            ...
            regions = BOPTools.SplitAPI.slice(
                bb, fragmentEdgesL, "Standard", tolerance=0.0
            )
        ...
        while True:
            ...
            if self.EdgeTagDictArchive.values():
                if isinstance(list(self.EdgeTagDictArchive.values())[0], dict):
                    none, tagArchive = self.getEdgeTagDictArchiveTagIndex(
                        fp, tag=None, index=i
                    )
    ...
    def callParentToRebuildMasterSketchTags(self, fp):
        ...
        for archObject in foundParentArchObjects:
            if archObject.isDerivedFrom("App::Link"):
                ...
                ArchSketch.setPropertiesLinkCommon(
                    linkedObjProxy, linkedObj, archObject, mode="ODR"
                )
    ...
    def getUnsortedEdgesWidth(self, fp, propSetUuid=None):
        ...
        for j in range(0, len(fp.Geometry)):
            curWidthEdgeTagDictSync = self.getEdgeTagDictSyncWidth(
                fp, None, j, propSetUuid
            )
    ...
    def getUnsortedEdgesAlign(self, fp, propSetUuid=None):
        ...
        alignsList = []
        for j in range(0, len(fp.Geometry)):
            curAlignEdgeTagDictSync = self.getEdgeTagDictSyncAlign(
                fp, None, j, propSetUuid
            )
    ...
    def getEdgeTagDictSyncRoleStatus(
        self, fp, tag=None, index=None, role="wallAxis", propSetUuid=None
    ):
    ...
    def getEdgeTagDictSyncWallStatus(
        self, fp, tag=None, index=None, role="wallAxis", propSetUuid=None
    ):

        roleStatus = self.getEdgeTagDictSyncRoleStatus(
            fp, tag, index, role, propSetUuid
        )
        if roleStatus == True:
            wallAxisStatus = True
    ...
    def getEdgeTagDictSyncCurtainWallStatus(
        self, fp, tag=None, index=None, role="curtainWallAxis"
    ):
        if tag is not None:
    ...
    def getEdgeTagIndex(
        self, fp, tag=None, index=None, useEdgeTagDictSyncFindIndex=False
    ):
    ...
    def getWallBaseShapeEdgesInfo(self, fp, role="wallAxis", propSetUuid=None):
        ...
        for i in skGeom:
            if isinstance(i.Geometry, ArchSketch.GeomSupported):
                wallAxisStatus = self.getEdgeTagDictSyncWallStatus(
                    fp, tag=i.Tag, role="wallAxis", propSetUuid=propSetUuid
                )
                if wallAxisStatus:
    ...
    def getStructureBaseShapeWires(self, fp, role="slab", archsketchEdges=None):
        ...
        return {
            "slabWires": structureBaseShapeWires,
            "faceMaker": "Bullseye",
            "slabThickness": 250,
        }
    ...
    def getCurtainWallBaseShapeEdgesInfo(self, fp, role="curtainWallAxis"):
        ...
        curtainWallAxisDict = {
            k: v
            for (k, v) in self.EdgeTagDictSync.items()
            if v.get("curtainWallAxis", None) == True
        }
    ...
    def setProperties(self, fp):
        if not hasattr(fp, "InputShapeObj"):
            fp.addProperty(
                ...
                "InputShapeObj",
                "Voxel Properties",
                "Input Object with Shape to calculate Voxel",
            )
        if not hasattr(fp, "VoxelObj"):
            fp.addProperty(
                ...
                "VoxelObj",
                "Voxel Properties",
                "Input VoxelObj with Shape to calculate Voxel",
            )
        ...
        if not hasattr(fp, "Mode"):
            fp.addProperty(
                ...
                "Mode",
                "Voxel Properties",
                "Algorithm to deduce the disposition of Voxels in an Input Shape Object",
            )
        ...
        if not hasattr(fp, "pList"):
            fp.addProperty(
                ...
                "pList",
                "Referenced Object",
                "Placement List of Voxels as calculated",
                8,
            )
        if not hasattr(fp, "eCount"):
            fp.addProperty(
                ...
                "eCount",
                "Referenced Object",
                "Element Count of Voxels as calculated",
                8,
            )
    ...
    if mode == "Center":
        ...
        for xc in range(0, 2):
            ...
            for yc in range(0, 2):
                ...
                for zc in range(0, 2):
                    zpp = zp + zc * lz
                    if shapeObj.Shape.isInside(
                        FreeCAD.Vector(xpp, ypp, zpp), tolerance, True
                    ):
                        return True
    # Test all 8 'corners' of the 'voxel'
        ...
        for xc in range(0, 2):
            ...
            for yc in range(0, 2):
                ...
                for zc in range(0, 2):
                    zpp = zp + zc * lz
                    if not shapeObj.Shape.isInside(
                        FreeCAD.Vector(xpp, ypp, zpp), tolerance, True
                    ):
                        return False
    ...
    def setProperties(self, fp):
        if not hasattr(fp, "InputShapeObj"):
            fp.addProperty(
                ...
                "Can select an Input Object with CellComplexElements before initial this command",
            )
...
class _CommandEditWallAlign:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Edit_Align.svg",
            "Accel": "E, A",
...
class GuiEditWallAlignObserver(SketchArchCommands.selectObjectObserver):
    ...
    def __init__(self, targetWall, targetBaseSketch):
        ...
        if True:  # if not targetWall.ArchSketchData:
            ...
            tempOverrideAlign = [
                i if i is not None else wallAlign for i in tempOverrideAlign
            ]
    ...
    def proceed(self, doc, obj, sub, pnt):
        ...
        if self.targetWall.ArchSketchData and hasattr(
            targetArchSk.Proxy, "getEdgeTagDictSyncAlign"
        ):
            curAlign = targetArchSk.Proxy.getEdgeTagDictSyncAlign(
                self.targetArchSketch, None, subIndex, propSetUuid=self.propSetUuid
            )
            if curAlign == "Left":
                curAlign = "Right"
            elif curAlign == "Right":
            ...
            if self.targetWall.ArchSketchData and not hasattr(
                targetArchSk.Proxy, "getEdgeTagDictSyncAlign"
            ):
                msg1 = (
                    "ArchSketchData set but SketchArch Not installed : "
                    + "This tool would update ArchWall's ArchSketchEdges "
                    + "but Not data in the underlying ArchSketch"
                )
                reply = QtGui.QMessageBox.information(None, "", msg1)
...
class _CommandEditWallWidth:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Edit_Width.svg",
            "Accel": "E, D",
...
class GuiEditWallWidthObserver(SketchArchCommands.selectObjectObserver):
    ...
    def __init__(self, targetWall, targetBaseSketch):
        ...
        if not targetWall.ArchSketchData:
            ...
            tempOverrideWidth = [
                i if i is not None else wallWidth for i in tempOverrideWidth
            ]
    ...
    def proceed(self, doc, obj, sub, pnt):
        ...
        if self.targetWall.ArchSketchData:
            curWidth = targetArchSk.Proxy.getEdgeTagDictSyncWidth(
                targetArchSk, None, subIndex, propSetUuid=self.propSetUuid
            )
            if not curWidth:
        ...
        reply = QtGui.QInputDialog.getText(
            None, "Input Width", "Width of Wall Segment", text=str(curWidth)
        )
...
class _CommandEditWallOffset:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Edit_Offset.svg",
            "Accel": "E, F",
...
class GuiEditWallOffsetObserver(SketchArchCommands.selectObjectObserver):
    ...
    def __init__(self, targetWall, targetBaseSketch):
        ...
        if not targetWall.ArchSketchData:
            ...
            tempOverrideOffset = [
                i if i is not None else wallOffset for i in tempOverrideOffset
            ]
    ...
    def proceed(self, doc, obj, sub, pnt):
        ...
        reply = QtGui.QInputDialog.getText(
            None, "Input Offset", "Offset of Wall Segment", text=str(curOffset)
        )
        if reply[1]:  # user clicked OK
            if reply[0]:
...
class _CommandEditWallAttach:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Edit_Attach.svg",
            "Accel": "E, T",
    ...
    def Activated(self):
        try:
            ...
            reply = QtGui.QMessageBox.information(
                None,
                "",
                "Select an ArchSketch or Arch Window/Equipment, Click this Button, and select the edge to attach ",
            )
            return
        try:
        ...
        if Draft.getType(sel0.getLinkedObject()) not in [
            "ArchSketch",
            "Window",
            "Equipment",
        ]:
        ...
        if sel1:
            if Draft.getType(sel1) != "Wall":
                reply = QtGui.QMessageBox.information(
                    None, "", "Target Object is Not Wall - Feature not supported 'Yet' "
                )
                return
            else:
                targetHostWall = sel1
        ...
        if not targetHostWall:
            if (
                Draft.getType(sel0.getLinkedObject()) != "ArchSketch"
            ):  # ArchSketch can has no hostWall / attach to (Arch)Sketch directly
                reply = QtGui.QMessageBox.information(
                    None,
                    "",
                    "Select a Window/Equipment with Host which is Arch Wall (or Select a Window/Equipment with an ArchWall as 2nd selection)",
                )
                return
        elif Draft.getType(targetHostWall) != "Wall":
            reply = QtGui.QMessageBox.information(
                None, "", "Window/Equipment's Host needs to be a Wall to function"
            )
            return
        ...
        if not targetBaseSketch:
            reply = QtGui.QMessageBox.information(
                None,
                "",
                "Wall needs to have Base which is to be Sketch or ArchSketch to function; ArchSketch needs to have a MasterSketch to function",
            )
            return
        if Draft.getType(targetBaseSketch) in ["ArchSketch", "Sketcher::SketchObject"]:
            ...
            App.Console.PrintMessage(
                "Select target Edge of the ArchSketch / Sketch to attach to " + "\n"
            )
            FreeCADGui.Selection.clearSelection()
            ...
            reply = QtGui.QMessageBox.information(
                None,
                "",
                "Gui to edit Arch Wall with a DWire Base is not implemented yet - Please directly edit ArchWall OverrideAlign attribute for the purpose.",
            )
        else:
            reply = QtGui.QMessageBox.information(
                None,
                "",
                "Wall needs to have Base which is to be Sketch or ArchSketch to function",
            )
            return
...
class _CommandEditStructure:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Edit_Structure_Toggle.svg",
            "Accel": "E, S",
    ...
    def Activated(self):
        try:
            ...
            reply = QtGui.QMessageBox.information(
                None,
                "",
                "Select an Arch Structure ( with underlying Base ArchSketch Sketch ) or ArchSketch ",
            )
            return
        targetObjectBase = None
        ...
        if Draft.getType(sel0) not in ["Structure"]:
            if Draft.getType(sel0) in ["ArchSketch"]:
                reply = QtGui.QMessageBox.information(
                    None,
                    "",
                    "Select an Arch Structure (with underlying Base ArchSketch Sketch) to Edit - Editing an ArchSketch directly would be implemented in due course",
                )
                return
            else:
                reply = QtGui.QMessageBox.information(
                    None,
                    "",
                    "Select an Arch Structure (with underlying Base ArchSketch Sketch) to Edit",
                )
                return
        if hasattr(sel0, "Base"):  # Wall has Base, ArchSketch does not
            if sel0.Base:
                ...
                reply = QtGui.QMessageBox.information(
                    None,
                    "",
                    "Arch Structure without Base is not supported - Select an Arch Structure ( with underlying Base ArchSketch Sketch ) or ArchSketch ",
                )
                return
        else:
            targetObjectBase = sel0
        ...
        if Draft.getType(targetObjectBase) in ["ArchSketch"]:
            ...
            Gui.ActiveDocument.setEdit(targetObjectBase)
            App.Console.PrintMessage(
                "Select target Edge of the ArchSketch to turn it on/off as Structure edges "
                + "\n"
            )
            FreeCADGui.Selection.clearSelection()
...
class GuiEditStructureObserver(SketchArchCommands.selectObjectObserver):
    ...
    def escape(self, info):
        k = info["Key"]
        if k == "ESCAPE":
            self.targetStructure.ViewObject.Transparency = (
                self.targetStructureTransparency
            )
            SketchArchCommands.selectObjectObserver.escape(self, info)
...
class _CommandEditCurtainWall:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath()
    ...
    def Activated(self):
        try:
            ...
            reply = QtGui.QMessageBox.information(
                None,
                "",
                "Select an Arch Curtain Wall ( with underlying Base ArchSketch Sketch ) or ArchSketch ",
            )
            return
        targetObjectBase = None
        ...
        if Draft.getType(sel0) in ["ArchSketch"]:
            ...
            if sel0.Base and Draft.getType(sel0.Base) in ["ArchSketch"]:
                ...
                reply = QtGui.QMessageBox.information(
                    None,
                    "",
                    "Arch Curtain Wall without Base is not supported - Select an Arch Curtain Wall ( with underlying Base ArchSketch Sketch ) or ArchSketch ",
                )
                return
        else:
            reply = QtGui.QMessageBox.information(
        ...
        if True:
            ...
            Gui.ActiveDocument.setEdit(targetObjectBase)
            App.Console.PrintMessage(
                "Select target Edge of the ArchSketch to turn it on/off as Curtain Wall edges "
                + "\n"
            )
            FreeCADGui.Selection.clearSelection()
...
class GuiEditCurtainWallObserver(SketchArchCommands.selectObjectObserver):
    ...
    def proceed(self, doc, obj, sub, pnt):
        ...
        if hasattr(self.targetArchSketch.Proxy, "getEdgeTagDictSyncCurtainWallStatus"):
            curCurtainWallStatus = (
                self.targetArchSketch.Proxy.getEdgeTagDictSyncCurtainWallStatus(
                    self.targetArchSketch, None, subIndex, role="curtainWallAxis"
                )
            )
            ...
            tempDict[self.targetArchSketch.Geometry[subIndex].Tag][
                "curtainWallAxis"
            ] = newCurtainWallStatus
            ...
            if curtainWallDict:
                for k, v in curtainWallDict.items():
                    i, none = self.targetArchSketch.Proxy.getEdgeTagIndex(
                        self.targetArchSketch, k, None
                    )
                    if i != None:
...
class _CommandEditWall:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Edit_Wall_Toggle.svg",
            "Accel": "E, W",
            "MenuText": "Edit Wall",
...
class _CommandPropertySet:
    ...
    def GetResources(self):
        ...
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/PropertySet.svg",
            "Accel": "Alt+T",
    ...
    def Activated(self):
        ...
        changeName = False
        # if sel0 is Wall
        if (sel0 != targetObjectBase) and (sel0.ArchSketchPropertySet != "Default"):
            changeName = True
            psUuid = sel0.Proxy.ArchSkPropSetPickedUuid
        elif (sel0 == targetObjectBase) and (targetObjectBase.PropertySet != "Default"):
            changeName = True
            psUuid = targetObjectBase.Proxy.PropSetPickedUuid
        if changeName:
            psn = targetObjectBase.Proxy.getPropertySet(
                targetObjectBase, propSetUuid=psUuid
            )
            reply = QtGui.QInputDialog.getText(
                None, "Input PropertySet Name", msg3c, text=psn
            )
            if reply[1]:  # user clicked OK
                psn = reply[0]
        ...
        if sel0 != targetObjectBase:  # i.e. sel0 is Wall
            reply = QtGui.QInputDialog.getText(
                None, "Input PropertySet Name", msg3a, text=us
            )
        else:  # i.e. sel0 is not Wall, only ArchSketch
            reply = QtGui.QInputDialog.getText(
                None, "Input PropertySet Name", msg3b, text=us
            )
        if reply[1]:  # user clicked OK
            psn = reply[0]
        else:  # user clicked Cancel, reply [0] will be ""
...
class _Command_ArchSketch:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/SketchArchWorkbench.svg",
            "Accel": "Alt+S",
...
class _Command_Voxel:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/Voxel.svg",
            "Accel": "Alt+v",
    ...
    def Activated(self):
        try:
            ...
            reply = QtGui.QMessageBox.information(
                None, "", "Select an object with Shape to 'voxelise' "
            )
            return
        try:
            sel1 = Gui.Selection.getSelection()[1]
        ...
        lva.setLink(vp)
        lva.setExpression("ElementCount", "" + "<<" + vp.Label + ">>" + ".eCount")
        lva.setExpression("PlacementList", "" + "<<" + vp.Label + ">>" + ".pList")
        lva.recompute()
...
class _Command_CellComplex:
    ...
    def GetResources(self):
        return {
            "Pixmap": SketchArchIcon.getIconPath() + "/icons/CellComplex.svg",
            "Accel": "Alt+c",
    ...
    def Activated(self):
        ...
        try:
            ...
            reply = QtGui.QMessageBox.information(
                None, "", "Select an object with Shape to get CellComplexElement "
            )
...
def attachToMasterSketch(
    subject,
    target=None,
    subelement=None,
    attachmentOffset=None,
    zOffset="0",
    intersectingSubelement=None,
    mapMode="ObjectXY",
):
...
def updateAttachmentOffset(fp, linkFp=None, mode=None):
    ...
    if msSubelement and hostSkProxy:
        ...
        if newIndex > -1:
            ...
            none, newTag = ArchSketch.getEdgeTagIndex(
                hostSkProxy, hostSketch, None, newIndex
            )
    ...
    if msIntSubelement and hostSkProxy:
        ...
        if newIntIndex > -1:
            ...
            none, newIntTag = ArchSketch.getEdgeTagIndex(
                hostSkProxy, hostSketch, None, newIntIndex
            )
    ...
    if msSubelementTag:  # if has previous tag (then should no newTag)
        if mode == "ODR":
            if fp.MasterSketchSubelementIndex > -1:
                ...
                none, initialTag = ArchSketch.getEdgeTagIndex(
                    hostSkProxy, hostSketch, None, msSubelementIndex
                )
            ...
            msSubelementIndex, none = ArchSketch.getEdgeTagIndex(
                hostSkProxy, hostSketch, msSubelementTag, None
            )
    ...
    if msIntSubelementTag:  # if has previous tag (then should no newTag)
        if mode == "ODR":
            if fp.MasterSketchIntersectingSubelementIndex > -1:
                ...
                none, initialTag = ArchSketch.getEdgeTagIndex(
                    hostSkProxy, hostSketch, None, msIntSubelementIndex
                )
                if Draft.getType(fp) == "ArchSketch":
            ...
            msIntSubelementIndex, none = ArchSketch.getEdgeTagIndex(
                hostSkProxy, hostSketch, msIntSubelementTag, None
            )
            # sync index in record ('toponaming tolerent' process)
            if msIntSubelementIndex != None:
                fp.MasterSketchIntersectingSubelementIndex = msIntSubelementIndex
            else:  #  previous tag has no corresonding index
                if hasattr(fp, "Proxy"):  # ArchSketch/ArchObjects (Win/Equip)
                    if fp.Proxy.Type == "ArchSketch":
    ...
    if attachToSubelementOrOffset in ["Attach to Edge", "Attach To Edge & Alignment"]:
        if msSubelementIndex != None:  # Can't proceed if no index is found
            if (
                hostSketch
            ):  # only calculate 'offset' if hostSketch, otherwise, still proceed but 'offset' is kept to 'origin'
                edgeOffsetPointVector = getSketchEdgeOffsetPointVector(
                    fp,
                    hostSketch,
                    msSubelementIndex,
                    msSubelementSnapPreset,
                    msSubelementSnapCustom,
                    msSubelementOffset,
                    attachmentOffsetXyzAndRotation,
                    flipOffsetOriginToOtherEnd,
                    flip180Degree,
                ...
                if attachToSubelementOrOffset == "Attach To Edge & Alignment":
                    ...
                    # if flip180Degree:
                    if (
                        (flip180Degree and (attachmentAlignment == "WallLeft"))
                        or (not flip180Degree and (attachmentAlignment == "WallRight"))
                        or (flip180Degree and (attachmentAlignment == "Left"))
                        or (not flip180Degree and (attachmentAlignment == "Right"))
                    ):
                        edgeAngle = edgeAngle + math.pi
                    tempAttachmentOffset.Rotation.Angle = edgeAngle
                else:
                    tempAttachmentOffset.Rotation.Angle = (
                        attachmentOffsetXyzAndRotation.Rotation.Angle
                    )

                # Offset Parallel from Line Alignment
                masterSketchSubelementEdgeVec = getSketchEdgeVec(
                    hostSketch, msSubelementIndex
                )
                ...
                if attachmentAlignment in ["WallLeft", "WallRight"]:
                    if hasattr(hostSketch, "Proxy"):
                        if hasattr(hostSkProxy, "getEdgeTagDictSyncWidth") and hasattr(
                            hostSkProxy, "EdgeTagDictSync"
                        ):
                            msSubelementWidth = hostSkProxy.getEdgeTagDictSyncWidth(
                    ...
                    if msSubelementWidth in [zeroMM, None]:
                        if hostWall:
                            try:
                                msSubelementWidth = (
                                    hostWall.OverrideWidth[msSubelementIndex] * MM
                                )
                            except:
                            ...
                            print(
                                " something wrong ?  msSubelementWidth=0 : Case, ArchSketch on MasterSketch so no hostWall"
                            )

                    if hasattr(hostSketch, "Proxy"):
                        if hasattr(hostSkProxy, "getEdgeTagDictSyncAlign") and hasattr(
                            hostSkProxy, "EdgeTagDictSync"
                        ):
                            align = hostSkProxy.getEdgeTagDictSyncAlign(
                                hostSketch, None, msSubelementIndex
                            )
                            if not align:
                    ...
                    if align == None:
                        if hostWall:  # elif hostWall:
                            ...
                            print(
                                " something wrong ?  align=None : Case, ArchSketch on MasterSketch so no hostWall"
                            )

                offsetValue = 0
                if (
                    msSubelementWidth != None
                ):  # TODO If None, latter condition will result in exception	#zeroMM is 0 ->
                    if msSubelementWidth.Value != 0:
                        offsetValue = (
                            msSubelementWidth.Value
                        )  # + attachmentAlignmentOffset.Value
                if attachmentAlignment == "WallLeft":
                    if align == "Left":
                        offsetValue = (
                            attachmentAlignmentOffset.Value
                        )  # no need offsetValue (msSubelementWidth.Value)
                    elif align == "Right":
                ...
                if offsetValue != 0:
                    vOffsetH = DraftVecUtils.scaleTo(
                        masterSketchSubelementEdgeVec.cross(Vector(0, 0, 1)),
                        -offsetValue,
                    )  # -ve
    ...
    extraRotation = FreeCAD.Placement(
        App.Vector(0, 0, 0), App.Rotation(0, 0, 0)
    )  # , 0)
    if (
        fp.AttachmentOffsetExtraRotation == "X-Axis CCW90"
    ):  # [ "X-Axis CW90", "X-Axis CCW90", "X-Axis CW180", ]
        extraRotation = FreeCAD.Placement(
            App.Vector(0, 0, 0), App.Rotation(0, 0, 90)
        )  # ,winSketchPl.Base)
        ...
        extraRotation = FreeCAD.Placement(
            App.Vector(0, 0, 0), App.Rotation(0, 0, 180)
        )  # ,winSketchPl.Base)
    elif fp.AttachmentOffsetExtraRotation == "Y-Axis CW90":
        extraRotation = FreeCAD.Placement(
            App.Vector(0, 0, 0), App.Rotation(0, 90, 0)
        )  # ,winSketchPl.Base)
        ...
        extraRotation = FreeCAD.Placement(
            App.Vector(0, 0, 0), App.Rotation(0, 180, 0)
        )  # ,winSketchPl.Base)
        ...
        extraRotation = FreeCAD.Placement(
            App.Vector(0, 0, 0), App.Rotation(180, 0, 0)
        )  # ,winSketchPl.Base)
    ...
    if linkFp or not hasattr(fp, "AttachmentOffset"):  ## TODO or if hostWall ...
        ...
        if Draft.getType(fp.getLinkedObject()) == "Window":
            ...
            winSkRotation = FreeCAD.Placement(
                App.Vector(0, 0, 0), App.Rotation(0, 0, 90)
            )
            tempAttachmentOffset = tempAttachmentOffset.multiply(
                winSkRotation
            ).multiply(invWinSketchPl)
        originBase = App.Vector(0, 0, 0)
        if hostWall:
            hostWallPl = hostWall.Placement
            hostWallRotation = FreeCAD.Placement(
                App.Vector(0, 0, 0), hostWallPl.Rotation, originBase
            )
            ...
            hostObjectRotation = FreeCAD.Placement(
                App.Vector(0, 0, 0), hostObjectPl.Rotation, originBase
            )
...
def makeArchSketch(
    grp=None,
    label="ArchSketch__NAME",
    attachToAxisOrSketch=None,
    placementAxis_Or_masterSketch=None,
    copyFlag=None,
    visibility=None,
):
    name = "ArchSketch"
...
def getSketchEdgeOffsetPointVector(
    subject,
    masterSketch,
    subelementIndex,
    snapPreset=None,
        ...
        if geoindex2 is None:
            ...
            if not flipOffsetOriginToOtherEnd:
                if not offsetFromIntersectingSubelement:  # use 1st portion
                    edgePortionVec = intersectPoint.sub(
                        masterSketch.Geometry[geoindex].StartPoint
                    )
                else:  # use 2nd portion
                    edgePortionVec = intersectPoint.sub(
                        masterSketch.Geometry[geoindex].EndPoint
                    )
    ...
    totalAttachmentOffset = (
        snapPresetCustomLength + attachmentOffset.Value
    )  # float(attachmentOffset)
    if not flipOffsetOriginToOtherEnd:
        edgeOffsetPoint = masterSketch.Geometry[geoindex].value(totalAttachmentOffset)
    else:  # elif flipOffsetOriginToOtherEnd:
        edgeOffsetPoint = masterSketch.Geometry[geoindex].value(
            edgeLength - totalAttachmentOffset
        )
    if (
        intersectPoint
    ):  # i.e. - if offsetFromIntersectingSubelement and (geoindex2 is not None):
        if not flipOffsetOriginToOtherEnd:
            intOffsetVec = intersectPoint.sub(
                masterSketch.Geometry[geoindex].StartPoint
            )
        else:  # elif flipOffsetOriginToOtherEnd:
...
def getSketchSortedClEdgesOrder(sketch, archSketchEdges=None, propSetUuid=None):
    ...
    for c, i in enumerate(skGeom):
        if isinstance(i, ArchSketch.GeomSupported):
            ...
            elif hasattr(sketch, "Proxy") and hasattr(
                sketch.Proxy, "getEdgeTagDictSyncWallStatus"
            ):
                skProxy = sketch.Proxy
                wallAxisStatus = skProxy.getEdgeTagDictSyncWallStatus(
                    sketch, tag=i.Tag, role="wallAxis", propSetUuid=propSetUuid
                )
            else:
...
def boundBoxShape(obj, enlarge):
    ...
    rect = Part.makePlane(
        bb.XLength + 2 * enlarge,
        bb.YLength + 2 * enlarge,
        p.Base,
